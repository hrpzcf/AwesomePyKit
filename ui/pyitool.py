...
"如果指定了SPEC文件，则以下绝大部分项目文件及生成控制都将不生效。"))
        ...
        self.le_output_name.setToolTip(_translate("pyitool", "对应选项：-n, --name\n"
...
"每行一个模块名称，\"名称\"是指 Python 代码中 import 语句所使用的模块名称，区分大小写。"))
        self.label_27.setText(_translate("pyitool", "排除的模块："))
        self.pb_clear_exclude_module.setText(_translate("pyitool", "清空"))
        self.pte_exclude_modules.setToolTip(_translate("pyitool", "对应选项：--exclude-module\n"
"如果您不想让 Pyinstaller 打包某些模块，则请把模块名称填入此处，每行一个。\n"
"以上\"名称\"是指 Python 代码中 import 语句所使用的模块名称，区分大小写。"))
        self.label_3.setText(_translate("pyitool", "其他模块搜索目录："))
...
"将程序打包成单文件夹形式，exe 可执行程序在文件夹的首层目录内。"))
        self.rb_pack_to_one_dir.setText(_translate("pyitool", "打包成单目录"))
...
"勾选此选项则将\"附加信息\"标签页中的内容写入生成的 exe 文件，这些信息可以在查看 exe 文件属性时看见。"))
        self.cb_write_info_to_exec.setText(_translate("pyitool", "将“附加信息”标签页的内容写入生成的可执行文件"))
        self.cb_uac_admin.setToolTip(_translate("pyitool", "对应选项：--uac-admin\n"
"使打包的 exe 程序启动时请求提升权限，打包需要管理员权限才能正常工作的程序时，请勾选此选项。"))
        self.cb_uac_admin.setText(_translate("pyitool", "打包的程序启动时请求提升权限"))
...
"某些二进制文件经过 upx 压缩后可能无法运行，可将这些文件名添加到此处以使 upx 在压缩时略过。"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab_build_control), _translate("pyitool", "打包控制"))
...
"仅当您知道自己在做什么时，才使用此选项。"))
        ...
        self.cb_explorer_show.setText(_translate("pyitool", "在资源管理器中显示"))
        self.cb_delete_spec_file.setToolTip(_translate("pyitool", "打包成功后删除 Pyinstaller 生成的 .spec 文件，打包失败则不会执行删除操作。"))
        self.cb_delete_spec_file.setText(_translate("pyitool", "删除 spec 文件"))
        self.cb_delete_working_dir.setToolTip(_translate("pyitool", "打包成功后删除临时工作文件夹，打包失败则不会执行删除操作。\n"
"如果你没有自定义临时工作目录所在目录，则临时工作目录指的就是项目根目录下的 build 目录。"))
        self.cb_delete_working_dir.setText(_translate("pyitool", "删除临时工作文件夹"))
        self.groupBox_4.setTitle(_translate("pyitool", "工具配置管理"))
        self.label_28.setText(_translate("pyitool", "保存当前配置"))
        self.uiLineEdit_config_remark.setToolTip(_translate("pyitool", "当前配置是指“程序打包工具”所有复选框、输入框等控件的设置状态。\n"
"输入一个你容易记住的备注名称并点击“保存”按钮将这些设置保存下来以便在不同的打包项目之间快速切换。"))
        self.uiLineEdit_config_remark.setPlaceholderText(_translate("pyitool", "输入配置的备注名称"))
        self.uiPushButton_save_cur_config.setToolTip(_translate("pyitool", "将“程序打包工具”当前复选框、输入框等控件设置状态保存下来，以左侧输入框内容作为这个保存下来的配置的名称。"))
        self.uiPushButton_save_cur_config.setText(_translate("pyitool", "保存"))
        self.label_29.setText(_translate("pyitool", "已保存的配置"))
        self.uiComboBox_saved_config.setToolTip(_translate("pyitool", "所有已保存的配置，这里显示的是你在保存配置时输入的备注名称。"))
        self.uiPushButton_delete_config.setToolTip(_translate("pyitool", "将左侧下拉选择框选中的配置删除。"))
        self.uiPushButton_delete_config.setText(_translate("pyitool", "删除"))
        self.uiPushButton_apply_config.setToolTip(_translate("pyitool", "将左侧下拉选择框选中的配置内容应用到“程序打包”工具。"))
        self.uiPushButton_apply_config.setText(_translate("pyitool", "应用"))
...
"选择不同 Python 环境时，在该环境中也应安装运行项目所需的所有模块。"))
        self.pb_select_py_env.setToolTip(_translate("pyitool", "选择在不同 Python 环境中将程序打包成可执行文件。\n"
