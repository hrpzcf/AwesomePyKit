# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'packager.ui'
#
# Created by: PyQt5 UI code generator 5.15.1
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_PkgMgr(object):
    def setupUi(self, PkgMgr):
        PkgMgr.setObjectName("PkgMgr")
        PkgMgr.setWindowModality(QtCore.Qt.ApplicationModal)
        PkgMgr.resize(996, 694)
        font = QtGui.QFont()
        font.setFamily("Microsoft YaHei UI")
        PkgMgr.setFont(font)
        self.centralwidget = QtWidgets.QWidget(PkgMgr)
        self.centralwidget.setObjectName("centralwidget")
        self.horizontalLayout_4 = QtWidgets.QHBoxLayout(self.centralwidget)
        self.horizontalLayout_4.setObjectName("horizontalLayout_4")
        self.horizontalLayout_3 = QtWidgets.QHBoxLayout()
        self.horizontalLayout_3.setObjectName("horizontalLayout_3")
        self.verticalLayout_2 = QtWidgets.QVBoxLayout()
        self.verticalLayout_2.setObjectName("verticalLayout_2")
        self.lb_python_dir = QtWidgets.QLabel(self.centralwidget)
        self.lb_python_dir.setObjectName("lb_python_dir")
        self.verticalLayout_2.addWidget(self.lb_python_dir)
        self.li_py_env_list = QtWidgets.QListWidget(self.centralwidget)
        self.li_py_env_list.setObjectName("li_py_env_list")
        self.verticalLayout_2.addWidget(self.li_py_env_list)
        self.horizontalLayout = QtWidgets.QHBoxLayout()
        self.horizontalLayout.setObjectName("horizontalLayout")
        self.btn_autosearch = QtWidgets.QPushButton(self.centralwidget)
        self.btn_autosearch.setObjectName("btn_autosearch")
        self.horizontalLayout.addWidget(self.btn_autosearch)
        self.btn_addmanully = QtWidgets.QPushButton(self.centralwidget)
        self.btn_addmanully.setObjectName("btn_addmanully")
        self.horizontalLayout.addWidget(self.btn_addmanully)
        self.btn_delselected = QtWidgets.QPushButton(self.centralwidget)
        self.btn_delselected.setObjectName("btn_delselected")
        self.horizontalLayout.addWidget(self.btn_delselected)
        self.btn_clearexpired = QtWidgets.QPushButton(self.centralwidget)
        self.btn_clearexpired.setObjectName("btn_clearexpired")
        self.horizontalLayout.addWidget(self.btn_clearexpired)
        self.verticalLayout_2.addLayout(self.horizontalLayout)
        self.line = QtWidgets.QFrame(self.centralwidget)
        self.line.setFrameShape(QtWidgets.QFrame.HLine)
        self.line.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.line.setObjectName("line")
        self.verticalLayout_2.addWidget(self.line)
        spacerItem = QtWidgets.QSpacerItem(20, 20, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        self.verticalLayout_2.addItem(spacerItem)
        self.te_info_stream = QtWidgets.QTextEdit(self.centralwidget)
        self.te_info_stream.setFocusPolicy(QtCore.Qt.StrongFocus)
        self.te_info_stream.setContextMenuPolicy(QtCore.Qt.NoContextMenu)
        self.te_info_stream.setReadOnly(True)
        self.te_info_stream.setObjectName("te_info_stream")
        self.verticalLayout_2.addWidget(self.te_info_stream)
        self.verticalLayout_2.setStretch(1, 9)
        self.verticalLayout_2.setStretch(5, 5)
        self.horizontalLayout_3.addLayout(self.verticalLayout_2)
        self.verticalLayout = QtWidgets.QVBoxLayout()
        self.verticalLayout.setObjectName("verticalLayout")
        self.lb_installed_pkgs_info = QtWidgets.QLabel(self.centralwidget)
        self.lb_installed_pkgs_info.setObjectName("lb_installed_pkgs_info")
        self.verticalLayout.addWidget(self.lb_installed_pkgs_info)
        self.tw_installed_info = QtWidgets.QTableWidget(self.centralwidget)
        palette = QtGui.QPalette()
        brush = QtGui.QBrush(QtGui.QColor(0, 170, 255))
        brush.setStyle(QtCore.Qt.SolidPattern)
        palette.setBrush(QtGui.QPalette.Active, QtGui.QPalette.Highlight, brush)
        brush = QtGui.QBrush(QtGui.QColor(47, 47, 47))
        brush.setStyle(QtCore.Qt.SolidPattern)
        palette.setBrush(QtGui.QPalette.Active, QtGui.QPalette.HighlightedText, brush)
        brush = QtGui.QBrush(QtGui.QColor(0, 170, 255))
        brush.setStyle(QtCore.Qt.SolidPattern)
        palette.setBrush(QtGui.QPalette.Inactive, QtGui.QPalette.Highlight, brush)
        brush = QtGui.QBrush(QtGui.QColor(0, 0, 0))
        brush.setStyle(QtCore.Qt.SolidPattern)
        palette.setBrush(QtGui.QPalette.Inactive, QtGui.QPalette.HighlightedText, brush)
        brush = QtGui.QBrush(QtGui.QColor(190, 190, 190))
        brush.setStyle(QtCore.Qt.SolidPattern)
        palette.setBrush(QtGui.QPalette.Disabled, QtGui.QPalette.Highlight, brush)
        brush = QtGui.QBrush(QtGui.QColor(255, 255, 255))
        brush.setStyle(QtCore.Qt.SolidPattern)
        palette.setBrush(QtGui.QPalette.Disabled, QtGui.QPalette.HighlightedText, brush)
        self.tw_installed_info.setPalette(palette)
        self.tw_installed_info.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers)
        self.tw_installed_info.setSelectionMode(QtWidgets.QAbstractItemView.MultiSelection)
        self.tw_installed_info.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectRows)
        self.tw_installed_info.setCornerButtonEnabled(False)
        self.tw_installed_info.setColumnCount(4)
        self.tw_installed_info.setObjectName("tw_installed_info")
        self.tw_installed_info.setRowCount(0)
        item = QtWidgets.QTableWidgetItem()
        self.tw_installed_info.setHorizontalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        self.tw_installed_info.setHorizontalHeaderItem(1, item)
        item = QtWidgets.QTableWidgetItem()
        self.tw_installed_info.setHorizontalHeaderItem(2, item)
        item = QtWidgets.QTableWidgetItem()
        self.tw_installed_info.setHorizontalHeaderItem(3, item)
        self.tw_installed_info.horizontalHeader().setHighlightSections(False)
        self.tw_installed_info.verticalHeader().setHighlightSections(False)
        self.verticalLayout.addWidget(self.tw_installed_info)
        self.gridLayout = QtWidgets.QGridLayout()
        self.gridLayout.setObjectName("gridLayout")
        self.btn_check_for_updates = QtWidgets.QPushButton(self.centralwidget)
        self.btn_check_for_updates.setObjectName("btn_check_for_updates")
        self.gridLayout.addWidget(self.btn_check_for_updates, 0, 3, 1, 1)
        self.btn_install_package = QtWidgets.QPushButton(self.centralwidget)
        self.btn_install_package.setObjectName("btn_install_package")
        self.gridLayout.addWidget(self.btn_install_package, 1, 0, 1, 1)
        self.btn_uninstall_package = QtWidgets.QPushButton(self.centralwidget)
        self.btn_uninstall_package.setObjectName("btn_uninstall_package")
        self.gridLayout.addWidget(self.btn_uninstall_package, 1, 1, 1, 1)
        self.btn_up_grade_package = QtWidgets.QPushButton(self.centralwidget)
        self.btn_up_grade_package.setObjectName("btn_up_grade_package")
        self.gridLayout.addWidget(self.btn_up_grade_package, 1, 2, 1, 1)
        self.btn_up_grade_all = QtWidgets.QPushButton(self.centralwidget)
        self.btn_up_grade_all.setObjectName("btn_up_grade_all")
        self.gridLayout.addWidget(self.btn_up_grade_all, 1, 3, 1, 1)
        self.horizontalLayout_2 = QtWidgets.QHBoxLayout()
        self.horizontalLayout_2.setObjectName("horizontalLayout_2")
        self.label_2 = QtWidgets.QLabel(self.centralwidget)
        self.label_2.setText("")
        self.label_2.setObjectName("label_2")
        self.horizontalLayout_2.addWidget(self.label_2)
        self.cb_check_uncheck_all = QtWidgets.QCheckBox(self.centralwidget)
        self.cb_check_uncheck_all.setObjectName("cb_check_uncheck_all")
        self.horizontalLayout_2.addWidget(self.cb_check_uncheck_all)
        self.gridLayout.addLayout(self.horizontalLayout_2, 0, 2, 1, 1)
        self.lb_waiting = QtWidgets.QLabel(self.centralwidget)
        self.lb_waiting.setText("")
        self.lb_waiting.setObjectName("lb_waiting")
        self.gridLayout.addWidget(self.lb_waiting, 0, 0, 1, 2)
        self.verticalLayout.addLayout(self.gridLayout)
        self.horizontalLayout_3.addLayout(self.verticalLayout)
        self.horizontalLayout_3.setStretch(0, 3)
        self.horizontalLayout_3.setStretch(1, 4)
        self.horizontalLayout_4.addLayout(self.horizontalLayout_3)
        PkgMgr.setCentralWidget(self.centralwidget)
        self.statusbar = QtWidgets.QStatusBar(PkgMgr)
        self.statusbar.setObjectName("statusbar")
        PkgMgr.setStatusBar(self.statusbar)

        self.retranslateUi(PkgMgr)
        QtCore.QMetaObject.connectSlotsByName(PkgMgr)

    def retranslateUi(self, PkgMgr):
        _translate = QtCore.QCoreApplication.translate
        PkgMgr.setWindowTitle(_translate("PkgMgr", "包管理器"))
        self.lb_python_dir.setText(_translate("PkgMgr", "Python 目录："))
        self.btn_autosearch.setText(_translate("PkgMgr", "自动搜索"))
        self.btn_addmanully.setText(_translate("PkgMgr", "手动添加"))
        self.btn_delselected.setText(_translate("PkgMgr", "删除选中项"))
        self.btn_clearexpired.setText(_translate("PkgMgr", "清除失效项"))
        self.lb_installed_pkgs_info.setText(_translate("PkgMgr", "已安装的模块/包/库信息："))
        item = self.tw_installed_info.horizontalHeaderItem(0)
        item.setText(_translate("PkgMgr", "模块/包/库名称"))
        item = self.tw_installed_info.horizontalHeaderItem(1)
        item.setText(_translate("PkgMgr", "当前版本"))
        item = self.tw_installed_info.horizontalHeaderItem(2)
        item.setText(_translate("PkgMgr", "最新版本"))
        item = self.tw_installed_info.horizontalHeaderItem(3)
        item.setText(_translate("PkgMgr", "状态"))
        self.btn_check_for_updates.setText(_translate("PkgMgr", "检查更新"))
        self.btn_install_package.setText(_translate("PkgMgr", "安装"))
        self.btn_uninstall_package.setText(_translate("PkgMgr", "卸载"))
        self.btn_up_grade_package.setText(_translate("PkgMgr", "升级"))
        self.btn_up_grade_all.setText(_translate("PkgMgr", "全部升级"))
        self.cb_check_uncheck_all.setText(_translate("PkgMgr", "全选 / 取消"))
