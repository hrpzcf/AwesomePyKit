...
class Ui_PyInstallerTool(object):
    def setupUi(self, PyInstallerTool):
        ...
        self.verticalLayout_25 = QtWidgets.QVBoxLayout(self.tab_advanced)
        ...
        self.lb_running_gif.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignTop|QtCore.Qt.AlignTrailing)
        ...
        self.lb_running_tip.setAlignment(QtCore.Qt.AlignBottom|QtCore.Qt.AlignLeading|QtCore.Qt.AlignLeft)
...
"生成的可执行文件的附加信息，即鼠标悬停于exe文件上显示的信息，以及exe文件属性中的详细信息。"))
        ...
        self.groupBox.setToolTip(_translate("PyInstallerTool", "以调试模式打包后，运行生成的可执行文件时总是会显示控制台窗口，并打印一些调试信息用于帮助排错。"))
        ...
        self.cb_db_bootloader.setText(_translate("PyInstallerTool", "引导进度信息"))
        self.cb_db_noarchive.setToolTip(_translate("PyInstallerTool", "不将所有冻结的Python源文件作为档案存储在生成的可执行文件中，而是将它们作为文件存储在生成的输出目录中。"))
        self.cb_db_noarchive.setText(_translate("PyInstallerTool", "不捆绑源文件"))
        self.groupBox_2.setToolTip(_translate("PyInstallerTool", "以\"单文件\"模式打包的程序运行时提取库和支持文件的临时储存位置。\n"
...
"\"_MEIxxxxxx\"文件夹将在此处创建。仅当您知道自己在做什么时，才使用此选项。"))
        ...
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab_advanced), _translate("PyInstallerTool", "高级设置"))
