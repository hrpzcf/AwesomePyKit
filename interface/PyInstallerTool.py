...
"仅当系统环境变量PATH中有upx路径信息或在以下“upx可执行文件查找路径”指定了upx查找路径时，此项设置才生效。"))
        ...
        self.cb_clean_before_build.setToolTip(_translate("PyInstallerTool", "生成exe可执行文件前是否清空PYINSTALLER生成的临时文件夹(临时文件可用于下次加速生成)。"))
        self.cb_clean_before_build.setText(_translate("PyInstallerTool", "构建前清理缓存并删除临时文件"))
        self.cb_write_info_to_exec.setToolTip(_translate("PyInstallerTool", "勾选此选项则将\"附加信息\"标签页中的条目写入生成的exe文件。"))
        self.cb_write_info_to_exec.setText(_translate("PyInstallerTool", "将附加信息写入生成的可执行文件"))
...
"某些动态链接库或可执行文件经过UPX压缩后可能无法运行，可将这些文件名添加到此处以使UPX在压缩时略过。"))
        ...
        self.tab_file_ver_info.setToolTip(_translate("PyInstallerTool", "需要在\"生成控制\"中勾选\"将文件信息写入生成的可执行文件\"，此设置才生效。\n"
"生成的可执行文件的附加信息，即鼠标悬停于exe文件上显示的信息，以及exe文件属性中的详细信息。"))
        ...
        self.groupBox.setTitle(_translate("PyInstallerTool", "以调试模式打包程序"))
...
"要求源代码文件后缀名不能是 *.pyw，\"生成控制\"标签中的\"打包的程序运行时显示控制台\"选项必须勾选。"))
        self.cb_db_imports.setText(_translate("PyInstallerTool", "运行时输出模块导入信息"))
        self.cb_db_bootloader.setToolTip(_translate("PyInstallerTool", "使引导程序在初始化和启动捆绑的应用程序时发出进度消息，用于诊断导入丢失的问题。\n"
"要求源代码文件后缀名不能是 *.pyw，\"生成控制\"标签中的\"打包的程序运行时显示控制台\"选项必须勾选。"))
        self.cb_db_bootloader.setText(_translate("PyInstallerTool", "运行时输出引导进度信息"))
        self.cb_db_noarchive.setToolTip(_translate("PyInstallerTool", "不将所有冻结的源代码文件存档在生成的可执行文件中，而是将它们作为单独的文件存储在生成的输出目录中。"))
        self.cb_db_noarchive.setText(_translate("PyInstallerTool", "不将源文件存档至可执行文件中"))
...
"\"_MEIxxxxxx\"文件夹将在您指定的位置创建，仅当您知道自己在做什么时，才使用此选项。"))
        self.groupBox_2.setTitle(_translate("PyInstallerTool", "打包单文件运行时临时目录"))
        ...
        self.label_6.setToolTip(_translate("PyInstallerTool", "选择在不同PYTHON环境中将程序打包成可执行文件。\n"
...
"选择不同PYTHON环境时，在该环境中也应安装运行项目所需的所有模块。"))
        ...
        self.label_18.setText(_translate("PyInstallerTool", "级别："))
